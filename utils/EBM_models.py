...
def create_ebm(net_type, num_filters, num_channels = 3, patch_size=1):
    ...
    elif net_type == 'LightEBM':
        net = LightweightEBM(n_c=num_channels, n_f=num_filters)
    elif net_type == 'SuperLightEBM':
        net = SuperLightweightEBM(n_c=num_channels, n_f=num_filters)
...
class LightweightEBM(nn.Module):
    def __init__(self, n_c=3, n_f=24, leak=0.05):
        super(LightweightEBM, self).__init__()
        self.f = nn.Sequential(
            nn.Conv2d(n_c, n_f, 3, 1, 1),
            nn.LeakyReLU(leak),
            ...
            nn.LeakyReLU(leak),
            nn.Conv2d(n_f * 4, n_f * 4, 3, 2, 1),
            nn.LeakyReLU(leak),
            nn.Conv2d(n_f * 4, 1, 4, 1, 0))

    def forward(self, x):
        return self.f(x).squeeze()
    

class SuperLightweightEBM(nn.Module):
    def __init__(self, n_c=3, n_f=64, leak=0.05):
        super(SuperLightweightEBM, self).__init__()
        self.f = nn.Sequential(
            nn.Conv2d(n_c, n_f, 3, 1, 1),
            nn.LeakyReLU(leak),
            nn.Conv2d(n_f, n_f * 2, 3, 2, 1),
