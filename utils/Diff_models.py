...
def create_diff(net_type,nf,im_sz=32,channel_mult=(1,2,2,2),num_res_blocks=2):

    if net_type == 'DM_UNET':
        if im_sz == 32: net = get_unet_32(nf=nf, channel_mult=channel_mult)
        elif im_sz == 128: net = get_unet_128(nf=nf)
        else: raise ValueError(f'Invalid image size for diffusion model DDPM_UNET: {im_sz}')

    elif net_type == 'DM_CONV':
        net = DiffusionModel(in_channels=3, out_channels=3, nf=nf)
    elif net_type == 'DM_UNET_SMALL':
